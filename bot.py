...
def _ensure_ffmpeg():
    ...
    exe = os.path.join(base, "ffmpeg.exe" if platform.system().lower() == "windows" else "ffmpeg")
    if not os.path.isfile(exe):
        system = platform.system().lower()
        if system == "linux":
            archive = os.path.join(base, "ffmpeg.tar.xz")
            urllib.request.urlretrieve("https://johnvansickle.com/ffmpeg/releases/ffmpeg-release-amd64-static.tar.xz", archive)
            ...
            archive = os.path.join(base, "ffmpeg.zip")
            urllib.request.urlretrieve("https://www.gyan.dev/ffmpeg/builds/ffmpeg-release-essentials.zip", archive)
            ...
            raise RuntimeError("Unsupported operating system for automatic FFmpeg setup.")
...
def load_opus_library() -> None:
    ...
    candidates.extend(["libopus.so.0", "libopus.so", "opus", "libopus-0.dll", "opus.dll"])
